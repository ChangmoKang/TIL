...
* RAM(Random Access Memory)
  * 데이터의 읽기, 쓰기 가능
  * 전원을 끄면 기록한 내용이 사라짐
  * 주로 프로그램의 작업 영역으로 사용되고 DRAM, SRAM이 있음
* ROM(Read Only Memory)
  * 기본적으로 데이터 읽기 전용
  * 전원을 꺼도 내용이 사라지지 않음
  * 기동 시 시스템 체크나 BIOS 정보 등을 저장하는데 사용
...
메모리 공간은 주소라 불리는 번지(예를 들어, 0xF12F)를 붙여서 각 영역을 식별함



메인 메모리의 용도별 분류

* 힙(heap): 파일로부터 읽어들인 데이터 등과 같이 필요한 만큼만 확보하여 사용하는 메모리 영역
* 스택(stack): 프로그램 실행 중에 이용하는 변수의 내용을 일시적으로 저장하는 메모리
...
하드디스크 안에 페이징 파일(스왑 파일)이라는 파일을 작성하여 이 파일과 물리 메모리를 합쳐서 가상의 메모리 영역으로 간주함으로써 물리 메모리의 용량을 초과한 메모리를 다룰 수 있다.

> 스왑: 물리 메모리와 페이징 파일 사이에서 메모리상의 프로세스를 일시적으로 교환하는 것
...
* 리로케이션
  ...
  한 번 확보한 메모리 영역의 위치를 변경하는 것. 특히, 실행 중인 프로그램의 위치를 변경하는 것을 다이내믹 리로케이션 이라고 함

* 가비지 콜렉션

  힙 영역이 가득 차서 액세스 할 수 없을 때 불필요한 메모리 영역을 자동으로 해제하는 것
...
입출력 기기를 제어할 때 메인 메모리와 I/O 주소 공간을 공유하는 방식을 "메모리 맵 I/O"라고 한다.



#### DMA(Direct Memory Access)

CPU를 거치지 않고 입출력 장치와 메모리 사이에 직접 데이터를 주고받는 방법
...
시스템에 장애가 발생해도 큰 손실이 나지 않도록 기기를 최소 필요한 것 이상으로 마련하여 시스템 전체에 여유를 가지게 하는 것
...
* 듀얼 시스템
  ...
  > 동일한 처리를 여러 대에 동시에 작동시키는 구성으로, 한 대가 정상적으로 작동하면 시스템의 기능을 유지함

* 듀플렉스 시스템

  > 다른 기기를 예비로 대기시키는 구성으로, 예비 기기가 시스템의 기능을 유지합니다.
...
* RAID 0
  ...
  > 데이터를 분산하여 기록(스트라이핑). 속도는 빠르지만 리던던시는 없음

* RAID 1

  > 각각의 SSD에 동일한 데이터를 기록함(미러링). 고속은 아니지만 리던던시는 있음

* RAID 2

  > 오류 검출과 정정이 가능한 코드(해밍 코드)를 기록.

* RAID 3

  > 데이터 오류를 발견하는 전용 디스크를  준비함

* RAID 4

  > 스트라이핑에 의해 데이터를 분산하면서 패리티 전용 디스크를 준비합니다.

* RAID 5

  > 패리티를 분산시켜 하나의 디스크에 장애가 발생해도 다른 디스크의 패리티로 복원할 수 있도록 함
...
* TCP/IP 프로토콜
  ...
  > TCP는 데이터를 일정한 크기로 분할, IP는 TCP가 분할한 데이터에 송신처를 붙여 네트워크로 보냄
...
OS가 지원하는 계층(데이터 링크층, 네트워크층, 트랜스포트층)



### 방화벽

인터넷 접속 시 외부 네트워크로부터 액세스에 대해 보안을 확보하기 위해 특정한 액세를 제한하는 소프트웨어
...
인터넷 포함한 네트워크상에 있는 다양한 데이터와 애플리케이션을 데스크탑과 똑같이 취급할 수 있도록 하는 개념

.NET 애플리케이션은 .NET Framework에 포함되어 있는 공통 언어 런타임이라는 가상의 실행 시스템상에서 작동하기 때문에 CPU나 OS에서도 동일한 프로그램을 실행시킬 수 있다.



### 명령 프롬프트

배치 파일: 명령 프롬프트에서 실행할 명령을 텍스트 파일에 모아서 작성한 파일
...
파일이나 디렉토리에 대해 사용자나 그룹이 갖고 있는 일기, 쓰기, 실행 권한을 말한다.



명령 'ls -l'을 입력하면 퍼미션과 관련된 정보를 확인할 수 있다.

`drwxr-xr-x 1 창모 000000    0 11월 19 23:10  TIL/`

* 맨 처음의 d는 디렉토리를 뜻한다.
* 그 다음 세 글자(rwx)는 user 설정
* 그 다음 세 글자(-xr)는 group 설정
* 그 다음 세 글자(r-x)는 other 설정이다.
* `-`는 '권한 없음'을 나타낸다
...
명령과 명령을 `|`로 연결하여 앞 명령에서 얻어지는 결과를 뒷 명령에 전달하는 것

`ls | grep T`

> ls 명령으로 구한 결과인 데스크탑 데렉토리의 파일(디렉토리)명 목록이 grep 명령으로 전달되어 해당 목록 중에서 'T'라는 문자를 포함하는 행을 검사함



### 리다이렉트

표준 입력, 표준 출력, 오류 출력의 입출력 위치를 변경하는 기능
...
> ls 명령으로 조사한 TIL 디렉토리의 파일(디렉토리)명 목록의 정보를 'a.txt'에 저장함
...
> 5장까지는 괜찮았으나 뒤로 갈수록 (~~내 기준에~~)필요 없는 내용이 많이 나와서 아쉬웠다.
>
> 공룡 책이라고 불리는 책을 읽기 전에 OS를 쉽게 접해보고자 했으나 OS를 모르는 나조차도 너무 얕게 설명되어 있다고 느꼈다.
